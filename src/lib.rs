...
impl TypeMap {
	...
	pub fn insert<T: 'static>(&mut self, val: T) -> Option<T> {
		unsafe {
			self.insert_raw(TypeId::of::<T>(), Box::new(val))
	...
	pub fn contains<T: 'static>(&self) -> bool {
		unsafe {
			self.contains_raw(&TypeId::of::<T>())
		}
	}

	pub fn get<T: 'static>(&self) -> Option<&T> {
		unsafe {
			self.get_raw(&TypeId::of::<T>()).and_then(|boxed| boxed.downcast_ref())
		}
	}

	pub fn get_mut<T: 'static>(&mut self) -> Option<&mut T> {
		unsafe {
			self.get_mut_raw(&TypeId::of::<T>()).and_then(|boxed| boxed.downcast_mut())
		}
	}

	pub fn remove<T: 'static>(&mut self) -> Option<T> {
		unsafe {
			self.remove_raw(&TypeId::of::<T>()).and_then(|boxed| boxed.downcast().ok().map(|boxed| *boxed))
		}
	}

	pub unsafe fn insert_raw(&mut self, id: TypeId, val: Box<dyn Any>) -> Option<Box<dyn Any>> {
		self.map
			.get_or_insert_with(|| FxHashMap::default())
			.insert(id, val)
	}

	pub unsafe fn contains_raw(&self, id: &TypeId) -> bool {
		self.map.as_ref().and_then(|m| m.get(id)).is_some()
	}

	pub unsafe fn get_raw(&self, id: &TypeId) -> Option<&Box<dyn Any>> {
		self.map
			.as_ref()
			.and_then(|m| m.get(id))
	}

	pub unsafe fn get_mut_raw(&mut self, id: &TypeId) -> Option<&mut Box<dyn Any>> {
		self.map
			.as_mut()
			.and_then(|m| m.get_mut(id))
	}

	pub unsafe fn remove_raw(&mut self, id: &TypeId) -> Option<Box<dyn Any>> {
		self.map
			.as_mut()
